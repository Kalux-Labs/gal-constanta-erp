...
import {getCitiesStatic} from "@/lib/supabase/utils/cities/cities.static";
...
export default async function CountyPage({
    ...
    const {data, count} = await getProjects({
        page: page, perPage: perPage, search: search, countySlug: countySlug, citySlug: citySlug, allowUnassigned: true
    });
    ...
    return (
        <div className="max-w-4xl mx-auto mb-8 flex flex-col gap-4 px-4">
            ...
            <ProjectFilters counties={counties} cities={cities} search={search} county={county} city={city}/>
...
export async function generateStaticParams() {
    ...
    const results = await Promise.all(
        counties.map((county) => getCitiesStatic({
            onlyAllowed: true,
            countyId: county.id
        }))
    )

    return counties.flatMap((county, index) =>
        results[index]?.map((city) => ({
